...
This spec defines a comprehensive refactor of the contracts form to match the modern visual design shown in the reference images. The key insight from the images is that this implements a **UI display toggle system** where users can switch between viewing CPA and S&H configuration sections, while both contract types can be configured for the same client.

## Design Requirements from Reference Images

### Key Design Principle: UI Display Toggle System
- **Display Toggle Switches**: Users can toggle between viewing CPA or S&H configuration sections
- **Single Section Display**: Only one contract type section is visible at a time for clean UX
- **Both Types Configurable**: Clients can have both CPA and S&H contracts simultaneously
- **Clean Interface**: Focused view on one contract type at a time reduces cognitive load

### Visual Layout Structure
- **Toggle Switch Design**: Clean toggle switches for CPA and S&H section display
...
- **Equipment Management**: Different approaches for CPA (multiple equipment cards) vs S&H (single equipment info)
...
- "PLANOS DE CONTRATO" section header with info note: "O cliente pode ter um ou ambos os tipos de contrato (CPA e/ou S&H)"
...
- "INFORMAÇÃO DO EQUIPAMENTO" section with three fields: MODELO, Nº SÉRIE, SOFTWARE
...
**As a user**, I want to toggle between viewing CPA and S&H contract configuration sections, so that I can focus on configuring one contract type at a time while maintaining the ability to configure both types for the same client.
...
**As a user**, I want to configure CPA contracts with multiple equipment support and proper discount logic, so that I can set up comprehensive CPA service agreements.
...
**As a user**, I want to configure S&H contracts with single equipment information, so that I can set up software and hardware support agreements.
...
**As a user**, I want to select a payment method that applies to the contract configuration, so that I can complete the contract setup.
...
**As a user**, I want my configuration data to persist when switching between contract type views, so that I don't lose information when toggling between CPA and S&H sections.
...
- **Solution**: Implement toggle logic that controls section visibility without clearing data
...
- **Requirement**: Different equipment handling for CPA (multiple) vs S&H (single)
...
1. **User Experience**: Users can easily toggle between CPA and S&H sections while configuring both
