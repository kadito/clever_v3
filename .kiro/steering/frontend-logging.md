...
All logging on the client side (Vue components, composables, stores) must use JSON.stringify for nested objects to ensure complete object visibility during debugging and troubleshooting.
...
- For very large objects, consider logging specific properties:
  ```javascript
  console.log('Large object subset:', JSON.stringify({
    id: obj.id,
    status: obj.status,
...
This standard ensures that when debugging issues or providing logs for troubleshooting:
