...
export const useAgendamentosStore = defineStore('agendamentos', () => {
  ...
  const pendingTasks = computed(() => agendamentos.value.filter(a => !a.tarefaConcluida).length);

  const completedTasks = computed(() => agendamentos.value.filter(a => a.tarefaConcluida).length);

  const postponedTasks = computed(() => agendamentos.value.filter(a => a.houveAdiamento).length);

  const installationTasks = computed(() => agendamentos.value.filter(a => a.instalacao).length);
