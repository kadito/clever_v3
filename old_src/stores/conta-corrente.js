...
export const useContaCorrenteStore = defineStore('conta-corrente', () => {
  ...
  async function fetchContaCorrenteForYear(year = null) {
    ...
    try {
      ...
      console.log(`Fetched ${contaCorrente.value.length} conta corrente items for year ${targetYear}`)
    } catch (err) {
  ...
  async function createContaCorrente(year, contaCorrenteData) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Failed to create conta corrente item: ${response.statusText}`)
      }
  ...
  async function updateContaCorrente(year, id, contaCorrenteData) {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || `Failed to update conta corrente item: ${response.statusText}`)
      }
  ...
  async function deleteContaCorrente(year, id) {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || `Failed to delete conta corrente item: ${response.statusText}`)
      }
