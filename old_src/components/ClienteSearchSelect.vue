<template>
  <div class="cliente-search-select">
    ...
    >
      ...
      <div
        v-else-if="filteredClientes.length === 0 && !loading && searchQuery.length > 0"
        class="dropdown-item no-results"
      >
...
const handleSearch = async () => {
  ...
  const currentDisplayName = selectedCliente.value
    ? selectedCliente.value.nomeComercial ||
      selectedCliente.value.nomeEmpresa ||
      selectedCliente.value.name ||
      ''
    : '';
    ...
    searchTimeout.value = setTimeout(async () => {
      try {
        const response = await fetch(
          `/api/clientes/search?q=${encodeURIComponent(searchQuery.value)}`
        );
        if (response.ok) {
...
const selectCliente = async cliente => {
  ...
  const displayName =
    fullCliente.nomeComercial || fullCliente.nomeEmpresa || fullCliente.name || '';
...
watch(
  ...
  async (newValue, oldValue) => {
    ...
    let cliente = clientes.value.find(
      c => c.id === newValue || c.nomeComercial === newValue || c.nomeEmpresa === newValue
    );
...
onMounted(async () => {
  ...
  if (props.modelValue) {
    const cliente = clientes.value.find(
      c =>
        c.id === props.modelValue ||
        c.nomeComercial === props.modelValue ||
        c.nomeEmpresa === props.modelValue
    );
    if (cliente) {
      selectedCliente.value = cliente;
