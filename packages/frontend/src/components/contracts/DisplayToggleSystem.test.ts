...
describe('Display Toggle System', () => {
  ...
  it('should display both sections when both toggles are active', async () => {
    ...
    expect(contractTypeSections[0].findComponent({ name: 'CPAContractSection' }).exists()).toBe(
      true
    );

    // Second section should contain S&H
    expect(contractTypeSections[1].findComponent({ name: 'SHContractSection' }).exists()).toBe(
      true
    );
  });
  ...
  it('should group each contract type with its toggle for clear visual separation', async () => {
    ...
    expect(shContainer.findComponent({ name: 'DisplayToggleSwitch' }).props('title')).toBe(
