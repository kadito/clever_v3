...
describe('Portuguese Localization Verification', () => {
  ...
  describe('CPAEquipmentManager', () => {
    it('displays Portuguese equipment labels correctly', () => {
      ...
      expect(wrapper.text()).toContain(
        'O desconto aplica-se apenas aos equipamentos adicionais (2º, 3º, etc.). O primeiro equipamento não tem desconto.'
      );
    });
  });
  ...
  describe('EquipmentCard', () => {
    ...
    it('displays Portuguese placeholders correctly', () => {
      ...
      const observationsTextarea = wrapper.find(
        'textarea[placeholder="Observações sobre este equipamento..."]'
      );
  ...
  describe('Form Configuration Portuguese Text', () => {
    it('verifies Portuguese text in form sections configuration', async () => {
      ...
      expect(contractTypesSection?.description).toBe(
        'Configuração dos contratos CPA e S&H com sistema de alternância de visualização'
      );

      const additionalInfoSection = contractsFormSections.find(
        (s: any) => s.key === 'additionalInfo'
      );
    ...
    it('verifies Portuguese field labels', async () => {
      ...
      const contractTypesField = contractTypesSection?.fields.find(
        (f: any) => f.key === 'contractTypes'
      );
      ...
      const paymentField = additionalInfoSection?.fields.find(
        (f: any) => f.key === 'metodoPagamento'
      );
