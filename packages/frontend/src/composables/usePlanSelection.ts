...
import { getAvailablePlans, getPlanOptions, getPlanDetails, requiresDistance, shouldShowPriceTable, type ContractType } from '../services/planSelection'
...
export function usePlanSelection() {
  ...
  const showPriceTable = computed(() => shouldShowPriceTable(selectedContractType.value, selectedPlanId.value, selectedDistance.value))
  ...
  return {
    ...
    setContractType,
    setPlanId: (planId: string) => { selectedPlanId.value = planId },
    setDistance: (distance: string) => { selectedDistance.value = distance }
  }
}