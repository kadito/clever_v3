...
class WorkflowTester {
  ...
  testBuildOrder() {
    ...
    const testBuildSteps = (workflowFile, workflowType) => {
      ...
      const buildSteps = steps.filter(step => 
        step.name?.includes('Build') || step.run?.includes('build')
      );

      // Check that shared builds first
      const sharedStep = buildSteps.find(step => 
        step.run?.includes('@clever/shared build')
      );
      const frontendStep = buildSteps.find(step => 
        step.run?.includes('@clever/frontend build')
      );
      const backendStep = buildSteps.find(step => 
        step.run?.includes('@clever/backend build')
      );
    ...
    this.test('Test workflow has correct build order', () => {
      testBuildSteps('.github/workflows/deploy-test.yml', 'test');
    }, 'testWorkflow');

    this.test('Prod workflow has correct build order', () => {
  ...
  testEnvironmentConfiguration() {
    ...
    this.test('Wrangler.toml has test environment config', () => {
    ...
    this.test('Wrangler.toml has production environment config', () => {
      const content = fs.readFileSync('wrangler.toml', 'utf8');
    ...
    this.test('Production workflow uses --env production flag', () => {
      const content = fs.readFileSync('.github/workflows/deploy-prod.yml', 'utf8');
      if (!content.includes('--env production')) {
  ...
  testSecurityConfiguration() {
    ...
    this.test('Test workflow uses required secrets', () => {
      testSecretsUsage('.github/workflows/deploy-test.yml', 'test');
    }, 'testWorkflow');

    this.test('Prod workflow uses required secrets', () => {
  ...
  testErrorHandling() {
    ...
    const testErrorHandling = (workflowFile, workflowType) => {
      ...
      const hasFailureHandling = steps.some(step => 
        step.if === 'failure()' || step.run?.includes('exit 1')
      );
    ...
    this.test('Test workflow has error handling', () => {
      testErrorHandling('.github/workflows/deploy-test.yml', 'test');
    }, 'testWorkflow');

    this.test('Prod workflow has error handling', () => {
  ...
  testBuildProcess() {
    ...
    this.test('Dependencies install successfully', () => {
      try {
    ...
    this.test('Shared package builds successfully', () => {
      try {
    ...
    this.test('Frontend package builds successfully', () => {
      try {
    ...
    this.test('Backend package builds successfully', () => {
      try {
  ...
  testDeploymentVerification() {
    ...
    const testVerificationSteps = (workflowFile, workflowType) => {
      ...
      const hasVerification = steps.some(step => 
        step.name?.includes('Verify') || step.run?.includes('deployment')
      );
    ...
    this.test('Test workflow has deployment verification', () => {
      testVerificationSteps('.github/workflows/deploy-test.yml', 'test');
    }, 'testWorkflow');

    this.test('Prod workflow has deployment verification', () => {
  ...
  printResults() {
    ...
    this.log(`Production Workflow: ${this.results.prodWorkflow.passed}/${prodTotal} tests passed`, 
